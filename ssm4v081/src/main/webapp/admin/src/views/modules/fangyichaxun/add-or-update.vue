<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="风险等级" prop="fengxiandengji">
          <el-select v-model="ruleForm.fengxiandengji" placeholder="请选择风险等级">
            <el-option
                v-for="(item,index) in fengxiandengjiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="风险等级" prop="fengxiandengji">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="旅游建议" prop="lvyoujianyi">
          <el-input v-model="ruleForm.lvyoujianyi" 
              placeholder="旅游建议" clearable  :readonly="ro.lvyoujianyi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="旅游建议" prop="lvyoujianyi">
              <el-input v-model="ruleForm.lvyoujianyi" 
                placeholder="旅游建议" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fangyichaxun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fangyichaxunCrossAddOrUpdateFlag = false;
                  this.parent.search();
