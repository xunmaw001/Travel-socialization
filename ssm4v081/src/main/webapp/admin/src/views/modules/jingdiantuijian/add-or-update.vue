<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="景点类别" prop="jingdianleibie">
          <el-select v-model="ruleForm.jingdianleibie" placeholder="请选择景点类别">
            <el-option
                v-for="(item,index) in jingdianleibieOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="景点类别" prop="jingdianleibie">
	      <el-input v-model="ruleForm.jingdianleibie"
                placeholder="景点类别" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jingdianmingcheng = true;
          ...
          if(o=='jingdianleibie'){
            this.ruleForm.jingdianleibie = obj[o];
	    this.ro.jingdianleibie = true;
            continue;
          }
          if(o=='jingdiandizhi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jingdianfenlei/leibie`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jingdianleibieOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jingdiantuijian/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingdiantuijianCrossAddOrUpdateFlag = false;
                  this.parent.search();
